  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter specifically for Girls T-Shirts
        const filtered = data.filter(p => p.category && p.category.toLowerCase() === 'girls t-shirts');
  ...
  return (
    <>
      ...
      <div className="boys-hero animate-fade-in-up" style={{ 
        backgroundImage: 'url("https://images.unsplash.com/photo-1503342217505-b0a15ec3261c?q=80&w=2070&auto=format&fit=crop")',
        marginBottom: '2rem'
      }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '4rem 10%', color: 'white' }}>
          <h1 style={{ fontSize: '4.5rem', fontWeight: 800, margin: 0, letterSpacing: '-0.02em', textShadow: '0 2px 10px rgba(0,0,0,0.3)' }}>
            <span style={{color: '#e11d48'}}>GIRLS</span> T-SHIRTS
          </h1>
          <p style={{ fontSize: '1.2rem', fontWeight: 500, marginTop: '0.5rem', textShadow: '0 1px 5px rgba(0,0,0,0.3)' }}>Fresh looks for every day</p>
        </div>
      </div>
      ...
      <div className="container" style={{ paddingBottom: '5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
          ...
          <span style={{ color: 'black', fontWeight: 500 }}>Girls T-Shirts</span>
        </div>
